' ----------------------------------------------------------------------
'   Script for printing the contents of a movie directory to a .CSV file.
'   (c) 2024-2026 Markus J. Auvo
'
'   The script reads the meta data from movie files in several root
'   directories (e.g. M:\, N:\) including their respective subdirectories
'   and prints it to an output file (.CSV). Output file can then be used
'   as an imported data source.
'
'   In this case, the movies have been are organized into category folders
'   serving as main folders in the root directory. Category folders begin
'   with an underscore '_', e.g. "X:\_Comedy", "X:\_Horror", etc.
'
'   The script iterates through following video file types: avi, mkv, mp4
'
'   The output file will be placed into a given location with a given
'   name.
...
rootDirs.Add "K:\", "CINEMA 1"
rootDirs.Add "L:\", "CINEMA 2"

' The output file

Const outputDir =   "D:\[ MOVIE LIST ]\"
Const outputFile =  "_movies.csv"
...
Const MOVIE_COMMENTS =           24 ' Contains the plot of the movie
Const MOVIE_LENGTH =             27
Const MOVIE_FILE_NAME =         165 ' File name including the file extension
Const MOVIE_FOLDER_STRUCTURE =  193 ' Folder structure of the file w/o the file
Const MOVIE_FOLDER_CURRENT =    194 ' Current folder of the file
Const MOVIE_FULL_PATH =         196 ' Full path of the file
Const MOVIE_SUBTITLE =          217 ' Subtitle language, e.g. EN, FI, SE
Const MOVIE_DIRECTOR =          322 ' Movie director
Const MOVIE_FRAME_HEIGHT =      324 ' Frame height
Const MOVIE_FRAME_WIDTH =       326 ' Frame width
...
Function initializeOutputFile
        ...
        WriteToConsole "Initializing output file..."
        WriteToFile(COL_TITLE & ";" & COL_PLOT & ";" & COL_YEAR & ";" & COL_DIRECTOR & ";" & COL_GENRE & ";" & COL_LENGTH & ";" & COL_SIZE & ";" & COL_FILE_TYPE & ";" & COL_WIDTH & ";" & COL_HEIGHT & ";" & COL_SUBTITLE & ";" & COL_FOLDER & ";" & COL_FILENAME)
        WriteToConsoleNL "[OK]"
        ' WriteToConsoleNL ""
End Function
...
Function iterateThroughFiles(subfolder)
    ...
    For Each fileName in folderNamespace.Items
        If (LCase(Right(fileName,4))=".avi" OR LCase(Right(fileName,4))=".mkv" OR LCase(Right(fileName,4))=".mp4") Then 
            ...
            Dim moviePlot
            moviePlot = folderNamespace.GetDetailsOf(fileName, MOVIE_COMMENTS)

            Dim movieYear
            movieYear = folderNamespace.GetDetailsOf(fileName, MOVIE_YEAR)

            Dim movieDirector
            movieDirector = folderNamespace.GetDetailsOf(fileName, MOVIE_DIRECTOR)
            ...
            ' Write to file
            WriteToFile(movieTitle & ";" & moviePlot & ";" & movieYear & ";" & movieDirector & ";" & movieGenre & ";" & movieLength & ";" & movieSize & ";" & movieFileType & ";" & movieFrameWidth & ";" & movieFrameHeight & ";" & movieSubtitle & ";" & movieFolderStructure & ";" & movieFile)
...
'
' Display a completion message in a dialog box
'
